function dx = Debris_EOM(t,state,cram)
...
%Sun's position
lo = phi_o + nu_o*t;
lambda_o = Omega_plus_w + lo + (pi/180)*( (6892/3600)*sin(lo) + (72/3600)*sin(2*lo) );
ro = (149.619 - 2.499*cos(lo) - 0.021*cos(2*lo))*(10^6);

Xo = ro*cos(lambda_o);
Yo = ro*sin(lambda_o)*cos(epsilon);
Zo = ro*sin(lambda_o)*sin(epsilon);
...
phi_mp = nu_mp*t;
phi_ms = nu_ms*t;
L0 = phi_mp + phi_ma + (pi/180)*218.31617;
lm = phi_ma + (pi/180)*134.96292;
llm = phi_m + (pi/180)*357.5256;
Fm = phi_mp + phi_ma + phi_ms + (pi/180)*93.27283;
Dm = phi_mp + phi_ma - phi_m  + (pi/180)*297.85027;
...
lambda_m = L0 + (pi/180)*( (22640/3600)*sin(lm) + (769/3600)*sin(2*lm) - (4856/3600)*sin(lm - 2*Dm) +...
...
bm = (pi/180)*( (18520/3600)*sin(Fm + lambda_m - L0 + (pi/180)*((412/3600)*sin(2*Fm) + (541/3600)*sin(llm)) ) -...
...
Ym = -sin(epsilon)*sin(bm)*rm + cos(epsilon)*cos(bm)*sin(lambda_m)*rm;
Zm =  cos(epsilon)*sin(bm)*rm + cos(bm)*sin(epsilon)*sin(lambda_m)*rm;
...
%Earth's J2 terms
J2term1 = GMe*(Re^2)*sqrt(5)*C20/(2*magR2^(1./2));
J2term2 = 3/(magR2^2);
...
C22term1 = 5*GMe*(Re^2)*sqrt(15)*C22/(2*magr2^(7./2));
C22term2 = GMe*(Re^2)*sqrt(15)*C22/(magr2^(5./2));
fC22x = C22term1*x*(y^2 - x^2) + C22term2*x;
fC22y = C22term1*y*(y^2 - x^2) - C22term2*y;
fC22z = C22term1*z*(y^2 - x^2);

S22term1 = 5*GMe*(Re^2)*sqrt(15)*S22/(magr2^(7./2));
S22term2 = GMe*(Re^2)*sqrt(15)*S22/(magr2^(5./2));
